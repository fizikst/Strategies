...
import { CCIDynamic1, CCIDynamic1BotOptions } from './bot';
import { GeneticSchema, DebutMeta, BaseTransport, WorkingEnv } from '@debut/types';

const parameters: GeneticSchema<CCIDynamic1BotOptions> = {
...
const meta: DebutMeta = {
    ...
    score(bot: CCIDynamic1) {
        const report = bot.plugins.stats.report();

        return report.balance - (report.maxBalance * report.potentialDD) / 100;
    },

    stats(bot: CCIDynamic1) {
        return bot.plugins.stats.report();
    },

    async create(transport: BaseTransport, cfg: CCIDynamic1BotOptions, env: WorkingEnv) {
        const bot = new CCIDynamic1(transport, cfg);
    ...
    ticksFilter(cfg: CCIDynamic1BotOptions) {
        if (!cfg.from && !cfg.to) {
            return () => true;
        }
    ...
    validate(cfg: CCIDynamic1BotOptions) {
        return cfg;
    },
};
